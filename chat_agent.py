...
async def main():
    ...
    assistant_agent = AssistantAgent(
        ...
        system_message="You're helpful cute personal assistant.",
    )

    # Load state with files.
    # if os.path.exists("assistance_agent.json"):
    #     with open("assistance_agent.json", "r") as f:
    #         state = json.load(f)
    ...
    state = cache_store.get("assistance_agent")
    ...
    while True:
        ...
        response = await assistant_agent.on_messages(
            messages=[message], cancellation_token=cancellation_token
        )
        print(f"{response.chat_message.content}\n{response.chat_message.models_usage}")

    # with open("assistance_agent.json", "w") as f:
    #     state = await assistant_agent.save_state()
    #     json.dump(state, f)

    state = await assistant_agent.save_state()
    cache_store.set("assistance_agent", json.dumps(state))
